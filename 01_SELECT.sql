 ...
 * --> GRANT, REVOKE
 * TCL(TRANJECTION CONTROL LANGUAGE) : 트랜젝션 제어 
 ...
 *이때, 조회된 행들의 집합을 "RESULT SET"의 (조회 결과의 집합)이라고 한다.
 *
...
SELECT EMP_ID , EMP_NAME , SALARY , SALARY * 12 AS SAL_YEAR FROM EMPLOYEE;
 ...
 * 컬렴명 AS "별칭" : 별칭 띄어쓰기 O, 특수문자 O, 문자 O
 * AS는 생략 가능
 */

-- SELECT 조회 결과의 집합인 RESULT SET에 출력되는 컬럼명을 지정

SELECT SYSDATE -1, SYSDATE AS 현재시간, SYSDATE + 1 FROM DUAL; 
...
SELECT EMP_NAME, SALARY, '원 입니다' AS 단위 FROM EMPLOYEE; 
...
-- 4. ORDER BY 컬럼명 | 별칭 | 컬럼 순 [ASC 오름차순(기본값) | DESC 내림차순] [NULLS FIRST | LAST]
...
FROM EMPLOYEE
WHERE DEPT_CODE = 'D9'; -- 비교 연산자(=) / 대입연산자 (:=)
...
-- EMPLOYEE 테이블에서 입사일이 1900-01-01~ 1999-12-31 사이인 직원의
-- 이름, 입사일 조회
...
-- '_' 예시
-- 'A_' : A로 시작하는 두 글자 문자열
-- '_ _ _A' : A로 끝나는 네 글자 문자열
-- '_ _A_ _' : 세 번째 문자가 A인 다섯 글자 문자 
-- '_ _ _ _ _': 다섯 글자 문자
...
--WHERE EMP_NAME LIKE '전__'; 이름이 외자이거나 네 글자는 해당 안돼
...
OR DEPT_CODE IS NULL --14명 (부서코드 없는 2명 포함)
ORDER BY DEPT_CODE;
-- IS NULL
-- IS NOT NULL
