...
/* >> 다중행 서브쿼리 앞에서는 일반 비교연산자 사용 불가
 * 
	 *  SELECT DEPT_TITLE, SUM(SALARY)
		FROM EMPLOYEE 
		LEFT JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
		GROUP BY DEPT_TITLE
		HAVING SUM(SALARY) = 17700000 , 800000, 600000, 150000 
		--> XXXX
 * 
	 * 	SELECT DEPT_TITLE, SUM(SALARY)
			FROM EMPLOYEE 
			LEFT JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID)
			GROUP BY DEPT_TITLE
			HAVING SUM(SALARY) IN (17700000 , 800000, 600000, 150000 )
			--> OOOO
...
SELECT EMP_ID 
FROM EMPLOYEE e1
SELF JOIN EMPLOYEE e2 ON (MANAGER_ID = EMP_ID)
*/
...
-- 정렬을 하려고 할 때 RESULT SET에 대한 SELECT 구문이 또 필요함
...
-- ( > ANY ): 과장 급여 3개 중, 최소금액인 220 보다 더 큰 SALARY / 가장 작은 값보다 큰가?
-- ( < ANY ): 과장 급여 3개 중, 최대금액인 366 보다 더 작은 SALARY / 가장 큰 값보다 작은가?
...
-- 상관 쿼리는 메인커리가 사용하는 테이블값을 서브쿼리가 이용해서 결과를 만듦
...
WHERE SALARY > (SELECT AVG(SALARY)
				FROM EMPLOYEE SUB
				WHERE SUB.JOB_CODE = MAIN.JOB_CODE); --GROUP BY 의미
...
-- 1) MAIN 의 1 행의 DEPT_CODE를 SUB 대입
-- 2) SUB를 수행
...
--> SELECT 절에만 작성되는 단일행 서브쿼리


-- 모든 직원의 이름, 직급, 급여, 전체 사원 중 가장 높은 급여와의 차

SELECT EMP_NAME, JOB_CODE, SALARY, 
(SELECT MAX(SALARY) SALARY FROM EMPLOYEE) - SALARY AS 차
FROM EMPLOYEE;
