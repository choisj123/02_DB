/* 
[JOIN 용어 정리]
  오라클       	  	                                SQL : 1999표준(ANSI) --> 안시 표준으로 공부하기 !!!
----------------------------------------------------------------------------------------------------------------
...
-- 2) 연결에 사용할 두 컬럼명이 같은 경우 --> ON 대신 USING 사용
...
-- 3. 교차 조인(CROSS JOIN == CARTESIAN PRODUCT) --> 잘 안씀(참고 정도만!)
...
-- 4. 비등가 조인(NON EQUAL JOIN) --> 잘 안씀(참고 정도만!)
...
FROM EMPLOYEE
--JOIN JOB USING(JOB_CODE);
NATURAL JOIN DEPARTMENT; --잘못 조인하면 CROSS join 결과 조회됨 
...
JOIN LOCATION ON(LOCATION_ID = LOCAL_CODE)
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID);
-- SQL Error [904] [42000]: ORA-00904: "LOCATION_ID": invalid identifier
...
--AND e.JOB_CODE  = 'J6' AND l.LOCAL_CODE IN ('L1', 'L2', 'L3');
AND JOB_NAME = '대리' 
...
-- WHERE DEPT_CODE IN('D5', 'D6')
 --????LEFT JOIN / JOIN 결과값은 같은데 뭐가 더 나은지?
...
SELECT EMP_NAME 사원명, JOB_NAME 직급명, SALARY 급여, DECODE(BONUS , NULL, SALARY * 12, ((SALARY * BONUS)+SALARY) * 12 ) "연봉(보너스포함)"
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
JOIN SAL_GRADE ON(SALARY BETWEEN MIN_SAL AND MAX_SAL)
...
((NVL(BOUNUS, '0') * SALARY) + SALARY ) *12

SELECT EMP_NAME , JOB_NAME, SALARY, DECODE(BONUS, NULL, SALARY * 12, ((SALARY * BONUS) + SALARY) * 12) 
FROM EMPLOYEE
JOIN JOB USING(JOB_CODE)
JOIN SAL_GRADE USING(SAL_LEVEL)
WHERE SALARY > MIN_SAL;
...
SELECT EMP_NAME 사원명, DEPT_TITLE 부서명, LOCAL_NAME 지역명, decode(NATIONAL_CODE,'KO', '한국', 'JP', '일본') 국가명 --, NATIONAL_NAME
FROM EMPLOYEE 
JOIN DEPARTMENT ON(DEPT_ID = DEPT_CODE)
JOIN LOCATION ON (LOCATION_ID = LOCAL_CODE)
...
FROM EMPLOYEE e1 
RIGHT JOIN EMPLOYEE e2 USING(DEPT_CODE)
